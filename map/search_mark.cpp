...
int constexpr kGoodRatingZoomLevel = 1;
int constexpr kBadRatingZoomLevel = scales::GetUpperScale();
...
drape_ptr<df::UserPointMark::SymbolNameZoomInfo> SearchMarkPoint::GetSymbolNames() const
{
  ...
  if (IsBookingSpecialMark())
  {
    ...
    else
    {
      if (HasGoodRating())
      {
        symbol->emplace(kGoodRatingZoomLevel, symbolName);
      }
      else  // bad or no rating
      {
        symbol->emplace(1 /* zoomLevel */, kColoredmarkSmall);
        symbol->emplace(kBadRatingZoomLevel, symbolName);
      }
    }
  }
  else
  {
    symbol->emplace(1 /* zoomLevel */, kColoredmarkSmall);
    if (HasRating())
      symbol->emplace(kUGCBadgeMinZoomLevel, symbolName);
  }
  return symbol;
}
  ...
  if (IsBookingSpecialMark())
  {
    if (m_isPreparing)
    {
      if (m_isSelected)
      {
        ...
        badgeInfo->m_zoomInfo.emplace(1, badgeName);
        return badgeInfo;
      }
    }
    else if (!m_isAvailable)
    {
      if (HasReason() && m_isSelected)
      {
        ...
        badgeInfo->m_zoomInfo.emplace(1 /* zoomLevel */, badgeName);
        return badgeInfo;
      }
    }
    else if (HasPrice() || HasPricing() || m_isSelected)
    {
      ...
      {
        if (m_isSelected)
          badgeInfo->m_zoomInfo.emplace(1, kSelectionChipSmall);
...
drape_ptr<df::UserPointMark::SymbolOffsets> SearchMarkPoint::GetSymbolOffsets() const
{
  ...
  if (IsUGCMark())
  {
    // UGC mark is not centered into corresponding texture
...
{
  ProcessMarks([this, &id, &reasonKey](SearchMarkPoint * mark) -> base::ControlFlow
  {
    if (id != mark->GetFeatureID())
      return base::ControlFlow::Continue;
